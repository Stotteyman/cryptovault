...
export class CombatEngine {
  ...
  resolveTurn(player: Combatant, enemy: Combatant, action: string) {
...
export class LootGenerator {
  generateLoot(difficulty: number) {
...
export function spinSlots(wager: number) {
...
export function spinWheel(wager: number) {
  const roll = Math.random()
  if (roll < 0.4) {
